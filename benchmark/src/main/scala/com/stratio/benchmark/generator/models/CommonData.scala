...
trait CommonData {
  ...
  @tailrec
  final def generateCreditCard(current: String): String = {
  ...
  val clientIdCreditCard: Map[Int, String] =
    initClientIdCreditCard((1 to Range_client_id._2), Map())
  ...
  def initClientIdCreditCard(idClients: Seq[Int],
    ...
    if (idClients.size == 0) {
      ...
      val newClientIdCreditCard = clientIdCreditCard + (idClients.last -> generateCreditCard(""))
  ...
  : Map[Int, (Double, Double)] = {
    clientIdCreditCard.map(x => {
      val index = generateRandomInt(0, geolocations.size - 1)
...
case class Continent(name: String, planet: Planet)

case class Planet(name: String, galaxy: Galaxy)
